...
export function createHarmSynFile (harmSynDef: HarmSynRecord[], minRelevantAmplitude: number) : string {
   ...
   for (let p = 0; p < n; p++) {
      ...
      out += r.time.toFixed(6) + " " + buildSinSynComponentsString(r.f0, r.amplitudes, prevAmplitudes, nextAmplitudes, minRelevantAmplitude) + eol; }
   return out; }
...
function buildSinSynComponentsString (f0: number, amplitudes: Float64Array, nextAmplitudes: Float64Array | undefined, prevAmplitudes: Float64Array | undefined, minRelevantAmplitude: number) : string {
   ...
   for (let i = 2; i <= amplitudes.length; i++) {
      ...
      if (a < minRelevantAmplitude &&
            (prevA === undefined || !isFinite(prevA) || prevA < minRelevantAmplitude) &&
            (nextA === undefined || !isFinite(nextA) || nextA < minRelevantAmplitude)) {
         continue; }
      s += " *" + i + "/" + a.toFixed(2); }
   return s; }
